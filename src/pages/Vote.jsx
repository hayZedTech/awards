...
const Vote = () => {
  ...
  // NEW STATE: Track Google sign-in loading
  ...
  const handleLoginGoogle = async () => {
    // START CHANGE: Show spinner immediately on click
    ...
    // We don't turn off the spinner here because the redirect handles navigation away.
    // If the call fails before redirecting, we handle the error.
    if (error) {
        Swal.fire('Error', error.message, 'error');
        setIsSigningInGoogle(false); // Only turn off if the call fails immediately
    }
  };
  ...
  // CHANGE: Centralized spinner for initial loading
  if (loading) return (
    <div className=""> 
        <LoadingSpinner />
    </div>
      ...
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-4">
        <div className="max-w-md w-full bg-white rounded-2xl shadow-xl p-8 text-center">
          ...
          <button 
            ...
            disabled={isSigningInGoogle} // Disable while signing in
            className={`w-full flex items-center justify-center gap-3 bg-white border border-gray-300 text-gray-700 hover:bg-gray-50 font-semibold py-3 px-4 rounded-xl transition-all shadow-sm hover:shadow-md mb-4 ${isSigningInGoogle ? 'cursor-not-allowed opacity-70' : ''}`}
          >
            {isSigningInGoogle ? (
                <LoadingSpinner size={20} /> // SHOW GOOGLE SPINNER
            ) : (
                <>
                    ...
                    <span className='text-amber-900'>Sign in with Google</span>
                </>
            )}
          </button>
          ...
          <div className="flex flex-col gap-3">
            ...
            <button
              ...
              // CHANGE: Remove gap-2 when loading to center the spinner/text
            ...
            >
              {isSendingMagicLink ? (
                <>
                  {/* ADJUSTMENT: Spinner is already centered due to flex, no need for gap */}
